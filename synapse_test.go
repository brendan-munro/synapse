...
func testUserMiddleware() Constructor {
	return func(next HandlerFunc) HandlerFunc {
		return func(c context.Context, w http.ResponseWriter, r *http.Request) {
			next(context.WithValue(c, "user", "foobarbaz"), w, r)
		}
	}
}
...
func testHelloHandler(c context.Context, w http.ResponseWriter, _ *http.Request) {
	fmt.Fprintf(w, "Hello, %s!\n", c.Value("user"))
}

func TestNewChain(t *testing.T) {
	c1 := func(HandlerFunc) HandlerFunc {
		return nil
	}
	c2 := func(HandlerFunc) HandlerFunc {
		return nil
	}
	...
	if want, have := true, funcsEqual(chain[0], c1); want != have {
		t.Error("unexpected constructor [0]:", have)
	}
	if want, have := true, funcsEqual(chain[1], c2); want != have {
...
func TestChain_AppendAddsHandlerFuncsCorrectly(t *testing.T) {
	...
	if want, have := 1, len(chain); want != have {
		t.Error("unexpected len:", have)
	}
	if want, have := 3, len(newChain); want != have {
		t.Error("unexpected len:", have)
	}
...
func TestChain_AppendRespectsImmutability(t *testing.T) {
	...
	if one, two := &chain[0], &newChain[0]; one == two {
...
func TestChain_ExtendAddsHandlerFuncsCorrectly(t *testing.T) {
	...
	if want, have := 1, len(chain1); want != have {
		t.Error("unexpected len:", have)
	}
	if want, have := 2, len(chain2); want != have {
		t.Error("unexpected len:", have)
	}
	if want, have := 3, len(newChain); want != have {
		t.Error("unexpected len:", have)
